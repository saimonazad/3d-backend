...
class WebdWoocommerceBProducts{
	...
	?>
		<h2>EDIT SIMPLE PRODUCTS </h2>	
		<?php
		$response = wp_remote_get( site_url()."/wp-json/wp/v2/product/" );	
		if( is_wp_error( $response ) ) {
			return;
		}
		$posts = json_decode( wp_remote_retrieve_body( $response ) );		
		if( empty( $posts ) ) {
				print "<p class='warning'>No products found.</p>";		
		}else { ?>
				<p>
					...
					<span class='togglerMassive button button-secondary warning btn btn-danger '>
						Massive Update <i class='fa fa-edit '></i>
					</span>
				</p>
				...
				<form name='editProductDisplay' id='editProductDisplay' method='post' action= "<?php echo admin_url( 'admin.php?page=webd-woocommerce-product-excel-importer-bulk-edit&tab=search-edit' ); ?>" >
					<table class='wp-list-table widefat fixed table table-bordered'>	
						...
						<?php if($_REQUEST['vocabularySelect']){	?>		
							<td>Tax Terms</td>
							<td>
							...
							if(count($terms) <1 ){
								...
								print "<option value='' >Choose Terms..</option>";
							...
							<tr>
								<td>Regular Price</td><td><input type='number' name='regular_price' id='regular_price' placeholder='Regular Price'/></td>
								<td>Regular Price Selector</td>
							<td>
							...
							<tr>
								<td>Products Per Page </td><td><input type='number' min="1" max="100" style='width:100%;' required name='posts_per_page' id='posts_per_page' placeholder='100 max' value='100'/></td><td></td><td></td>
							</tr>
							
							<tr>
								<td>Offset Products</td><td><input type='number' name='offset' min="1" max="100" style='width:100%;' id='offset' placeholder='Start from..'/></td>
								<td></td><td></td>
							</tr>
					...
					?>
					<table class='wp-list-table widefat fixed table table-bordered'>
						<legend><h2>FIELDS TO SHOW</h2></legend>
						...
						submit_button('Search','primary','Search'); 
						?>		
				</form>
			<?php
			...
			$query = new WP_Query( array(
				...
				<div class='table-wrapper' ><div class='table table-bordered convertToTable'>			
							...
							<?php
								...
								foreach($taxonomy_objects as $tax){									
									if(!strstr($tax->name,'pa')){
										if($_REQUEST["toShow".$tax->name] ){ //show columns according to what is checked
								...
								foreach($column_name as $d){
									...
									if($_REQUEST["toShow".strtolower(str_replace(" ","_",$d))]  ){
							...
							if ( $product->is_type( 'simple' ) ) {							
								...
								<?php } ?>
								<?php if($_REQUEST["toShowdescription"] ){ ?>
								<div>
								...
								<?php foreach($post_meta as $meta){
									if($_REQUEST["toShow".$meta] ){ /*show columns according to what is checked)*/ ?>
								...
								$terms = get_post_taxonomies( get_the_ID());
									...
									foreach($terms as $tax){									
										$term = get_the_terms( get_the_ID(), $tax );
											if($_REQUEST["toShow".$tax] ){//show columns according to what is checked
												if(!strstr($tax,'pa') ){
													...
													$myterm = array();
													while($i<$countTerms){
														array_push($myterm, esc_attr($term[$i]->name));
														$i++;
													}
													$terms = implode(',',$myterm);
													print "<input type='text' name='".esc_attr($tax)."' value='".esc_attr($terms)."' />";
													print "</div>";
		...
		?><div>
			<form name='updateMassive' id='updateMassive' method='post' action= "<?php echo admin_url( 'admin.php?page=webd-woocommerce-product-excel-importer-bulk-edit&tab=search-edit' ); ?>">
				...
				<tr>
					...
					<td><label for='regular_price'>Regular Price</label></td>
					<td><input step="0.01"  type='number' id='bulk_regular_price' name='regular_price' value='<?php print $this->regular_price; ?>' /></td>
					<td><label for='regular_operator'>How to change Regular Price</label></td>
					<td>
						
						<select disabled name='regular_operator' id='regular_operator'>
							<option value='ea'> Exact Amount</option>
							<option value='mfa'>Minus fixed amount</option>
							<option value='pfa'>Plus fixed amount</option>
							<option value='mpa'>Minus % amount</option>
							<option value='ppa'>Plus % amount</option>
						</select>
					</td>					
				</tr>
				...
				<tr>
					...
					<td><label for='sale_operator'>How to change Sale Price</label></td>
					<td>						
...
print "<tr><td><label for='".str_replace('_',' ',esc_attr($voc->name))."'>". str_replace('_',' ',esc_attr($voc->name)). "</label></td>";							
	...
	public function updateMassive(){
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST' && current_user_can('administrator') && $_REQUEST['updateMassive'] ){
				...
				foreach($id_Array as $id){
					...
					if(!empty($_REQUEST['regular_price']) ){
	...
	public function updateFromList(){
			if($_SERVER['REQUEST_METHOD'] == 'POST' && current_user_can('administrator') && $_REQUEST['submitList'] ){
				check_admin_referer( 'updateProductList' );
				check_ajax_referer( 'updateProductList' );	

				$this->id = sanitize_text_field($_POST['id']);
				
				if(!empty($_POST['_sku']))update_post_meta( $this->id, '_sku', sanitize_text_field($_POST['_sku']) );
				if(!empty($_POST['_regular_price']))update_post_meta( $this->id, '_regular_price', sanitize_text_field($_POST['_regular_price']) );
				...
				if(!empty($_POST['_stock']))update_post_meta( $this->id, '_stock', sanitize_text_field($_POST['_stock']) );
				if(!empty($_POST['_stock_status']))update_post_meta( $this->id, '_stock_status', sanitize_text_field($_POST['_stock_status']) );
				if(!empty($_POST['_length']))update_post_meta( $this->id, '_length', sanitize_text_field($_POST['_length']) );
				if(!empty($_POST['_width']))update_post_meta( $this->id, '_width', sanitize_text_field($_POST['_width']) );
				if(!empty($_POST['_height']))update_post_meta( $this->id, '_height', sanitize_text_field($_POST['_height']) );
				if(!empty($_POST['_weight']))update_post_meta( $this->id, '_weight',sanitize_text_field($_POST['_weight']) );
