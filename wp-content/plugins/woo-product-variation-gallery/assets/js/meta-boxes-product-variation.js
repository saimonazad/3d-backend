  ...
  var wc_meta_boxes_product_variations_actions = {
    ...
    variations_loaded: function variations_loaded(event, needsUpdate) {
      ...
      if (!needsUpdate) {
        ...
        $('input.variable_is_downloadable, input.variable_is_virtual, input.variable_manage_stock', wrapper).change(); // Open sale schedule fields when have some sale price date
        ...
        $('.woocommerce_variations .variation-needs-update', wrapper).removeClass('variation-needs-update'); // Disable cancel and save buttons
  ...
  var wc_meta_boxes_product_variations_ajax = {
    ...
    save_changes: function save_changes(callback) {
      var wrapper = $('#variable_product_options').find('.woocommerce_variations'),
          ...
          data = {}; // Save only with products need update.
  ...
  var wc_meta_boxes_product_variations_pagenav = {
    ...
    update_variations_count: function update_variations_count(qty) {
      var wrapper = $('#variable_product_options').find('.woocommerce_variations'),
          ...
          displaying_num = $('.variations-pagenav .displaying-num'); // Set the new total of variations
    ...
    set_paginav: function set_paginav(qty) {
      var wrapper = $('#variable_product_options').find('.woocommerce_variations'),
          ...
          options = ''; // Set the new total of pages

      wrapper.attr('data-total_pages', total_pages);
      $('.total-pages', page_nav).text(total_pages); // Set the new pagenav options

      for (var i = 1; i <= total_pages; i++) {
        options += '<option value="' + i + '">' + i + '</option>';
      }

      $('.page-selector', page_nav).empty().html(options); // Show/hide pagenav
      ...
      if (0 === new_qty) {
        ...
        $('.variation_actions').val('add_variation'); // Show/hide links
