...
class SettingsAPI {
	...
	public function switch_field_callback( $args ) {
		...
		return sprintf( '<fieldset><label class="rtwpvg-switch"><input %1$s type="checkbox" id="%2$s-field" name="%4$s[%2$s]" value="%3$s" %5$s/><span class="rtwpvg-switch-slider round"></span></label>%6$s</fieldset>',
			...
			$this->setting_id,
			checked( $value, true, false ),
			isset( $args['desc'] ) && $args['desc'] ? '<p class="description">' . esc_attr( $args['desc'] ) . '</p>' : null
		); 
	}
	...
	public function image_field_callback( $args ) {
        ...
        $h .= sprintf("</div>
                    ...
                    $name, 
                    esc_html__( 'Upload Image', 'woo-product-variation-swatches' ));
        $h .= "</div>";
	...
	function pro_alert_html() { 
        ...
        $html .= '<div class="rtvg-document-box rtvg-alert rtvg-pro-alert">
                ...
                <div class="rtvg-box-content">
                    <h3 class="rtvg-box-title">' . esc_html__( 'Pro field alert!', 'woo-product-variation-swatches' ) . '</h3>
                    ...
                    <a href="https://www.radiustheme.com/downloads/woocommerce-variation-images-gallery/?utm_source=WordPress&utm_medium=gallery&utm_campaign=pro_click" target="_blank" class="rt-admin-btn">' . esc_html__("Upgrade to pro", "woo-product-variation-swatches") . '</a>
