...
class ewdotpOrdersTable extends WP_List_Table {
	...
	public function advanced_filters() {
		...
		$views = apply_filters( 'ewd_otp_orders_table_views_date_range', $views );
		?>
...
<?php
	...
	public function column_default( $order, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'updated' :
				$value = esc_html( $order->status_updated_fmtd );
				break;
		...
		foreach ( $ids as $id ) {
			...
			elseif ( isset( $action ) ) {
				...
				foreach ( $statuses as $status ) {
					...
					if ( sanitize_title( $status->status ) != $action ) { continue; }
		...
		<div id="ewd-otp-admin-notice-bulk-<?php esc_attr( $this->last_action ); ?>" class="updated">
			...
			<?php elseif ( isset( $this->last_action ) ) : ?>
				...
				<?php foreach ( $statuses as $status ) { ?>
					...
					<?php if ( sanitize_title( $status->status ) != $this->last_action ) { continue; } ?>
	...
	public function display_tablenav( $which ) {
		...
		<div class="ewd-otp-table-header-controls">
			...
			<?php endif; ?>
			<div class='ewd-otp-admin-table-filter-div'>
	...
	public function orders_data() {
		...
		if ( $this->filter_order_status ) { 
			...
			foreach ( $statuses as $status ) {
				...
				if ( $this->filter_order_status != sanitize_title( $status->status ) ) { continue; }
